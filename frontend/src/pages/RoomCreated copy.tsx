...
import { useRoomActions } from '@/hooks/roomcreated/useRoomActions';
    ...
    <div className="bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 min-h-screen transition-all duration-1000">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full border border-white/30 hover:bg-white/30 transition-all duration-300">
              <span className="text-white text-sm font-semibold">
                Salon : <span className="text-yellow-300 font-bold">{room.code}</span>
              </span>
            </div>
            <button
                  ...
                  <div className="space-y-3">
                    {!gameState?.teams?.red?.disciples || gameState.teams.red.disciples.length === 0 ? (
                      ...
                      gameState?.teams?.red?.disciples?.map((disciple) => (
                        <div key={`red-disciple-${disciple.id}`}>{renderUserCard(disciple, 'red', 'disciple')}</div>
                      ))
                    )}
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {gameState?.teams?.blue?.disciples?.length === 0 ? (
                      ...
                      gameState?.teams?.blue?.disciples?.map((disciple) => (
                        <div key={`blue-disciple-${disciple.id}`}>{renderUserCard(disciple, 'blue', 'disciple')}</div>
                      ))
                    )}
                  </div>
                </div>
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-white/20 hover:bg-white/15 transition-all duration-300">
                ...
                <div className="space-y-3">
                  {!gameState?.spectators || gameState.spectators.length === 0 ? (
                    ...
                    <>
                      {(() => {
                        // Affiche tous les IDs pour repérer les doublons (log uniquement)
                        const spectatorIds = gameState.spectators.map((s) => s.id);
                        console.log('Spectator IDs:', spectatorIds);

                        // Supprime les doublons basés sur l'id
                        const uniqueSpectators = gameState.spectators.filter(
                          (s, i, arr) => i === arr.findIndex((t) => t.id === s.id)
                        );

                        return uniqueSpectators.map((spectator) => (
                          <div key={`spectator-${spectator.id}`}>
                    ...
                    <div className="bg-white rounded-xl p-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <span
                            className={`ml-2 font-semibold ${debugInfo.userExists ? 'text-green-600' : 'text-red-600'}`}
                          >
              ...
              {!debugInfo && (
                <div className="bg-gray-50 rounded-2xl p-6 text-center">
                  <p className="text-gray-600">
                    Cliquez sur les boutons ci-dessus pour obtenir des informations de debug
                  </p>
                </div>
              )}
            </div>
      ...
      <footer className="relative z-10 px-6 py-4">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-white/60 text-sm">© 2025 Kensho – Salon de jeu en temps réel</p>
        </div>
      </footer>
    </div>
  );
