...
const Room: React.FC<RoomProps> = ({ room, currentUser, onSendMessage, onLeaveRoom, socket }) => {
  ...
  const copyRoomCode = () => {
    const gameUrl = `${window.location.origin}?code=${room.code}`;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full border border-white/30">
              <span className="text-white text-sm font-semibold">Admin : <span className="text-green-300 font-bold">{creator?.username || 'N/A'}</span></span>
            </div>
          </div>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-gray-600 text-sm">
                            {index === 0 ? 'Créateur du salon' : 'Participant'}
                          </p>
                        </div>
                      </div>
