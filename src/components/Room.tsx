...
import { GameParameters } from '../types'; // Ajuste le chemin selon ton arborescence
...
const Room: React.FC<RoomProps> = ({ room, currentUser, onSendMessage, onLeaveRoom, socket }) => {
  ...
  // 3. Tous les useState et hooks ici, DANS cette fonction
  const [gameParameters, setGameParameters] = useState<GameParameters | null>(null);
...
*/
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on('gameParametersSet', (params: GameParameters) => {
        console.log('Paramètres de jeu reçus :', params);
  ...
  console.log('Room component - Current user:', currentUser);
  console.log('Room component - Is admin:', isAdmin);
  console.log('Room component - Room users:', room.users);
  ...
  return (
    <div className="bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 min-h-screen transition-all duration-1000">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full border border-white/30">
              <span className="text-white text-sm font-semibold">Créateur : <span className="text-green-300 font-bold">{room.users[0]?.username || 'N/A'}</span></span>
            </div>
          </div>
      ...
      {/* Parameters Modal*/}
      {showParametersModal && gameParameters && (
        <div
          className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
          onClick={() => setShowParametersModal(false)} // fermer si clic en dehors
        >
          <div
            className="bg-white p-6 rounded-lg max-w-md w-full"
            onClick={e => e.stopPropagation()} // éviter la fermeture quand clic dans la boîte
          >
            <h2 className="text-xl font-bold mb-4">Paramètres du salon</h2>
            <div className="space-y-2">
              <div>Temps phase 1 : {gameParameters.ParametersTimeFirst}s</div>
              <div>Temps phase 2 : {gameParameters.ParametersTimeSecond}s</div>
              <div>Temps phase 3 : {gameParameters.ParametersTimeThird}s</div>
              <div>Nombre de rerolls : {gameParameters.ParametersTeamReroll}</div>
              <div>Nombre de mots interdits : {gameParameters.ParametersTeamMaxForbiddenWords}</div>
              <div>Nombre de propositions : {gameParameters.ParametersTeamMaxPropositions}</div>
              <div>Score max pour gagner : {gameParameters.ParametersPointsMaxScore}</div>
              <div>Règle d’attribution : {gameParameters.ParametersPointsRules === 'tie' ? 'Égalité autorisée' : 'Pas d’égalité'}</div>
              <div>Difficultés mots sélectionnées : {
                  [
                    gameParameters.ParametersWordsListSelection.veryCommon && 'Très courants',
                    gameParameters.ParametersWordsListSelection.lessCommon && 'Moyens',
                    gameParameters.ParametersWordsListSelection.rarelyCommon && 'Rares'
                  ].filter(Boolean).join(', ')
                }
              </div>
            </div>
